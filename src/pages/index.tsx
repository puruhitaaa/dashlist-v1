...
const Home: NextPage = () => {
  const { data, isLoading, isError, refetch } = trpc.todo.getTodos.useQuery();
    ...
    <>
      ...
      <main className="relative flex min-h-screen flex-col items-center justify-center space-y-10 bg-gradient-to-b from-[#2e026d] to-[#15162c]">
        ...
        {data.length ? (
          data.map((todo) => (
            <div
              className="relative flex h-14 w-full max-w-[640px] cursor-pointer items-center justify-center rounded-2xl bg-white p-5 text-xl font-bold text-black outline-none transition-colors ease-out hover:bg-slate-200"
              key={todo.id}
            >
              {todo.isDone ? null : (
                <CheckCircleIcon
